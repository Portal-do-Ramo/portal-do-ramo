...
class AdicionarAssessorProjetoRequest extends FormRequest
{
    ...
    {
        return [
            'matricula_assessor' => ['bail', 'present', 'nullable', new Matricula, Rule::exists('usuarios', 'matricula')->where(fn($query) => $query->where('situacao_id', '<>', 3)), new MatriculaAssessor($this->route('projeto')->assessor->matricula)],
        ];
    }
